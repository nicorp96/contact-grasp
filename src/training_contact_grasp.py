...
class MyTrainer(TrainerWithTest):
    ...
    def train_report(self, epoch, step):
        ...
        print(
            f"    {epoch:03d}/{step:05d}:  loss: {self.loss:6.8f} , acc: {self.avg_acc:6.8f} "  # , miou: {self.m_iou:6.4f} , mcc: {self.mcc:6.4f}"
        )
...
class MainTrainingContactGrasp:
    NUM_SHOW_DS = 5
    FOLDER_VALIDATION = "validation_data"

    def __init__(self, config_path) -> None:
        self._config = configparser.ConfigParser()
        self._config.read_dict(open_json(config_path))
        self._device = "cuda:0"
        # self._contact_net_pcl_ds_test = ContactNetPCDatasetTest("data_pcl_contact")
        ...
        #     # # shuffle=True,
        #     # pin_memory=True,
        #     # num_workers=self._config["training"].getint("num_workers"),
        # )
        self._contact_grasp = ContactGraspModel(
        ...
        if show:
            for index in np.random.choice(
                ...
                (
                    ...
                    label,
                    contact_points,
                ) = self._contact_net_pcl_ds_train[index]
                point_cloud = o3d.geometry.PointCloud()
                point_cloud.points = o3d.utility.Vector3dVector(points[:, :3])
                point_cloud.paint_uniform_color([0, 0, 0])
                contact_grasp_pc = o3d.geometry.PointCloud()
                idx_true = label[0, :] == 1.0
                points_grasp = points[:, :3][idx_true]

                idx_points_true = contact_points[0, :] == 1.0
                points_grasp_contact = points[:, :3][idx_points_true]

                contact_grasp_pc = o3d.geometry.PointCloud(
                    points=o3d.utility.Vector3dVector(points_grasp)
                )
                contact_grasp_pc.paint_uniform_color([1, 0, 0])

                contact_grasp_points_pc = o3d.geometry.PointCloud(
                    points=o3d.utility.Vector3dVector(points_grasp_contact)
                )
                contact_grasp_points_pc.paint_uniform_color([0, 0, 1])
                o3d.visualization.draw_geometries(
                    [point_cloud, contact_grasp_pc, contact_grasp_points_pc]
                )
    ...
    def run_training(self):
        ...
        tr.summary()
        hist = tr.train()
        torch.save(tr.mdl.state_dict(), "contact_grasp_weights.pth")
